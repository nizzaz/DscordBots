...
party_positions = {}
new_startup = True
connection = mysql.connector.connect(host='localhost', database='RPEngine', user='REDACTED', password='REDACTED')


def reconnect_db():
    global connection
    if connection is None or not connection.is_connected():
        connection = mysql.connector.connect(host='localhost', database='RPEngine', user='REDACTED',
                                             password='REDACTED')
    return connection
            ...
            if message.content != 'CONFIRM':
                ...
                for delete in deletions:
                    await commit_sql("""DELETE FROM """ + delete + """ WHERE ServerId=%s;""",
                                     (str(dm_tracker[message.author.id]["server_id"]),))
                result = await commit_sql(
                    """INSERT INTO GuildSettings (ServerId,GuildBankBalance,StartingHealth,StartingMana,StartingStamina,StartingAttack,StartingDefense,StartingMagicAttack,StartingAgility,StartingIntellect,StartingCharisma,HealthLevelAdd,ManaLevelAdd,StaminaLevelAdd,XPLevelRatio,HealthAutoHeal,ManaAutoHeal,StaminaAutoHeal,AutoCharApproval) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);""",
                    (str(dm_tracker[message.author.id]["server_id"]), "1000000", "200", "100", "100", "10", "5", "10",
                     "10", "10", "10", "200", "100", "100", "200", "0.05", "0.1", "0.1","1"))
            ...
            #  await dm_tracker[message.author.id]["commandchannel"].send(">>> " + server_monsters[server_id]["MonsterName"] + " dodged the attack! No damage taken!")
                ...
                if server_monsters[server_id]["Health"] < 1:
                    ...
                    for user in server_party_chars[server_id].keys():
                        ...
                        if server_monsters[server_id]["Experience"] is not 0:
        ...
        if command == 'help' or command == 'info':
            ...
            if parsed_string == 'setup':
                ...
                if help_command == 'createroles':
                    ...
                    params = "None"
                    example = "=pets"
                elif help_commands == 'encounterstats':
                ...
                if weapon is not None:
                    ...
                    #  await dm_tracker[message.author.id]["commandchannel"].send(">>> " + server_monsters[server_id]["MonsterName"] + " dodged the attack! No damage taken!")
                        ...
                        if records:
                            ...
                            break           
                counter = counter + 1
            if server_monsters[message.guild.id] > 1:    
                await monster_attack(message.author.id)
