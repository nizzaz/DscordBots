    ...
    if message.content.startswith('~'):
        ...
        if (command == 'sayhi'):
            ...
            if re.search("game over", game_text, re.IGNORECASE):
                ...
                if not records:
                    result = await commit_sql("""INSERT INTO Stats (ServerId, UserId, FiveRuns) VALUES (%s, %s, %s);""",(str(message.guild.id),str(message.author.id),'1'))
                else:
                    for row in records:
